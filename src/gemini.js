...
export async function generateReply(history, mode = 'reply', customerName = null, gender = null, opts = {}) {
  ...
  try {
    ...
    if (mode === 'aftercare') {
      contents.push({
        role: 'user',
        parts: [{ text: 'MODE: AFTERCARE — Đây là BỆNH NHÂN ĐÃ ĐẾN KHÁM tại phòng khám (đọc thẻ [BN_CŨ]/[KHÁCH ĐÃ KHÁM] và làm ĐÚNG theo thẻ [QUY TRÌNH CHĂM SÓC] nếu có). Nhiệm vụ: chăm sóc sau khám — xưng đúng tên, hỏi thăm ĐÚNG triệu chứng bệnh họ điều trị, dặn dò đúng loại điều trị đã làm, dẫn tới mốc kế tiếp (buổi tiếp theo / tái khám) khi hợp ngữ cảnh. CẤM: xin số điện thoại, mời "suất tư vấn miễn phí", gửi link sale page, nói "Trợ lý Bác sĩ sẽ gọi lại trong ít phút" (trừ khi khách CHỦ ĐỘNG xin được gọi). Khách vừa bấm "Chia sẻ thông tin" qua OA → cảm ơn đã kết nối hồ sơ, rồi HỎI THĂM tình trạng, đừng trả lời như vừa nhận số của lead. ⚠️ Nếu bệnh nhân kêu NẢN / muốn BỎ liệu trình ("tiêm nhiều quá thôi bỏ cuộc"), chê "không ai tư vấn lúc khám", hoặc than chỉ nhận tin nhắn → đặt handover=true NGAY, chỉ 1–2 tin đồng cảm + hứa Bác sĩ đích thân gọi lại; CẤM giảng "theo đủ liệu trình" quá 1 lần trong hội thoại, CẤM dọa "bỏ ngang bệnh nặng hơn" — càng thuyết phục kiểu trả bài khách càng bỏ hẳn (ca Bé Tuyết 03/08). ⚠️ Bệnh nhân hỏi Y LỆNH RIÊNG của Bác sĩ (loại đai/nẹp nào, tên thuốc, liều, số buổi...) mà thẻ hồ sơ KHÔNG ghi rõ → TUYỆT ĐỐI không hỏi ngược bệnh nhân ("mình quên loại nào đúng không ạ?" = đổ việc nhớ cho người bệnh, ca Duy Cường 20/08 khách gắt "hồ sơ khám bác sĩ nắm mà"), không đoán bừa — trả lời đúng 1 câu "để em hỏi lại Bác sĩ Trình rồi nhắn mình liền nha" + đặt handover=true, handover_reason ghi rõ câu cần Bác sĩ trả lời. Đặt phone_captured=false. Mỗi lượt tối đa 2 tin ngắn.' }],
      });
    }
