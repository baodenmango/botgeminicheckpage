...
// ⏰ GIỜ VÀNG cho chạm CHỦ ĐỘNG (anh Trình chốt 11/07: "khách nhắn 11h đêm, bot rep hoài lúc
// 1-2-3h sáng thì rất khó chịu"). Chặn MỌI chạm tự động ngoài 8h–22h VN — cả bot tự gửi LẪN báo
// telesale (báo lúc 2h sáng người cũng ngủ, vô ích). Lead ngoài giờ KHÔNG markTouchDone → grace 6h
// + lượt cron sáng bắn lại. LƯU Ý: đây chỉ chặn chạm THEO LỊCH; khách nhắn đêm thì handleMessage
// (đường đối đáp khác) vẫn trả lời bình thường. Chỉnh qua TOUCH_GIO_TU/DEN.
const gioVN = () => new Date(Date.now() + 7 * 3600 * 1000).getUTCHours();
const GIO_TU = parseInt(process.env.TOUCH_GIO_TU || '8', 10);
const GIO_DEN = parseInt(process.env.TOUCH_GIO_DEN || '22', 10);
function trongGioVang() { const h = gioVN(); return h >= GIO_TU && h < GIO_DEN; }
...
export async function runSevenTouch() {
  ...
  // ⏰ Ngoài giờ vàng (8h–22h VN) → KHÔNG chạm chủ động lượt này. Thoát sớm, lead giữ nguyên
  // touch_done cũ → grace 6h + cron sáng bắn lại. Chặn cảnh bot nhắn/telesale báo lúc 1-3h sáng.
  if (!trongGioVang()) {
    console.log(`[7cham] ⏰ ${gioVN()}h VN ngoài khung ${GIO_TU}-${GIO_DEN}h → hoãn chạm chủ động tới sáng`);
    return 0;
  }
